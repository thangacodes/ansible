...
sleep 5
rm -rf apache-tomcat-9.0.64.tar.gz  #removing the tar.gz file, post downloads in /tmp directory
...
ls -l /opt/tomcat9/apache-tomcat-9.0.64/webapps/SampleWebApp > /tmp/SampleWebApp_files.txt
cat /opt/tomcat9/apache-tomcat-9.0.64/logs/catalina.out > /tmp/tomcat_log.txt
sh /opt/tomcat9/apache-tomcat-9.0.64/bin/shutdown.sh  #Tomcat service stopping
